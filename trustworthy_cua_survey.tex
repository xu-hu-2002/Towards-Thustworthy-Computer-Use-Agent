...
A trustworthy CUA must satisfy properties that go well beyond task-completion accuracy. Using the five trustworthiness dimensions introduced in Definition~2---\emph{security}, \emph{privacy}, \emph{reliability}, \emph{controllability}, and \emph{accountability}---we organize evaluation goals in a way that stays consistent with the survey's earlier boundary-by-dimension analysis. Table~\ref{tab:eval-goals} summarizes each goal together with representative metrics and the current benchmarks or evaluation artifacts that provide the strongest coverage.
...
\begin{longtable}{@{}L{3.10cm}L{10.95cm}@{}}
\caption{Evaluation goals for trustworthy CUA, representative metrics, and benchmark-level evidence coverage.}
\label{tab:eval-goals}\\
\toprule
\multicolumn{1}{c}{\textbf{Benchmark and Metrics}} & \multicolumn{1}{c}{\textbf{What It Covers}} \\
\midrule
\endfirsthead

\multicolumn{2}{@{}l}{\footnotesize Table~\ref{tab:eval-goals} (continued)}\\
\toprule
\multicolumn{1}{c}{\textbf{Benchmark and Metrics}} & \multicolumn{1}{c}{\textbf{What It Covers}} \\
\midrule
\endhead
...
\bottomrule
\multicolumn{2}{@{}p{\textwidth}@{}}{\footnotesize \emph{Note.} \openclawmark marks OpenClaw-specific benchmarks or evaluation artifacts.}\\
\endlastfoot

\multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Security}\par
\textit{Metrics}:\par
\textit{Attack-side}: Attack Success Rate (ASR), Response Rate (RespRate), Target ASR ($\mathrm{ASR}_{\mathrm{target}}$)\par
\textit{Defense-side}: Classification Accuracy (ClassAcc), False Positive Rate (FPR), Detection Rate (DetectRate), Block Rate (BlockRate)}\\
\addlinespace[2pt]
\openclawmark PASB~\cite{doubleagent2026} & End-to-end personalized OpenClaw benchmark for indirect prompt injection; also measures STM / LTM extraction and tampering. \\
SafeArena~\cite{safearena2025} & Harmful task completion, refusal behavior, and normalized safety across widely deployed base models. \\
OS-Harm~\cite{osharm2025} & Harmful or unsafe action execution in agentic computer-use settings. \\
WASP~\cite{wasp2025} & Content hijacking and prompt-injection-style steering through externally controlled interface content. \\
SecureWebArena~\cite{securewebarena2025} & Security-oriented web-agent evaluation with adversarial webpages and policy-sensitive execution. \\
OpenAgentSafety~\cite{openagentsafety2025} & Deployment-oriented safety tasks spanning prompt injection, social engineering, and multi-turn tool interactions. \\
\openclawmark ClawTrap~\cite{clawtrap2026} & MITM-based OpenClaw red-teaming for live traffic tampering and network-layer attack evaluation. \\
\openclawmark SafeClaw-R~\cite{safeclawr2026} & OpenClaw-specific defense-side evaluation for block / review / pass verdicts; reports 97.8\% detection and 3.4\% FPR. \\

\midrule
\multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Privacy}\par
\textit{Metrics}: Leakage Rate (LeakageRate); Element-level Privacy Recall; End-to-end Privacy Recognition Accuracy (E2E-Acc); Extraction Rate ($\mathrm{ExtractRate}_{\mathrm{m}}$); Write Success Rate ($\mathrm{WSR}_{\mathrm{m}}$)}\\
\addlinespace[2pt]
GUIGuard~\cite{guiguard2026} & Cross-platform visual privacy benchmark (630 trajectories, 13{,}830 screenshots) with region-level grounding, three risk levels, six privacy categories, and task-necessity labels; SOTA models achieve only 13.3\% (Android) / 1.4\% (PC) end-to-end accuracy. \\
Anon-Privacy~\cite{anonprivacy2026} & Visual anonymization for cloud-hosted GUI reasoning, with privacy-preserving task execution. \\
MLA-Trust~\cite{mlatrust2025} & Multi-step GUI-agent privacy and trust risks on high-risk interactive tasks. \\
\openclawmark PASB~\cite{doubleagent2026} & Exposes memory-privacy vulnerabilities via IPI attacks on STM / LTM in personalized OpenClaw settings; privacy coverage is indirect (attack-success metrics, not direct PII leakage measurement). \\
OpenAgentSafety~\cite{openagentsafety2025} & Partial privacy-relevant coverage through policy-sensitive safety tasks. \\

\midrule
\multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Reliability}\par
\textit{Metrics}: $k$-trial Pass Rate (Pass@$k(\tau,\varepsilon)$)}\\
\addlinespace[2pt]
ScreenSpot-Pro~\cite{screenspotpro2025} & High-resolution professional-UI grounding with small targets and dense layouts. \\
WorldGUI~\cite{worldgui2025} & Robustness under non-default initial states and desktop layout variation. \\
Online-Mind2Web~\cite{onlinemind2web2025} & Live-web reliability under dynamic loading and temporal content drift. \\
MemGUI-Bench~\cite{memguibench2026} & Cross-temporal and cross-spatial GUI memory demands in realistic mobile workflows. \\
AMA-Bench~\cite{amabench2025} & Agentic memory recall under realistic retrieval conditions. \\
CUARewardBench~\cite{cuarewardbench2025} & Reward-model calibration for memory-conditioned GUI trajectories. \\

\midrule
\multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Controllability}\par
\textit{Metrics}: Completion under Policy (CuP); Refusal Rate; Permission Violation Rate}\\
\addlinespace[2pt]
ST-WebAgentBench~\cite{stwebagentbench2025} & CuP metric for task completion under simultaneous policy compliance. \\
BeSafe-Bench~\cite{besafebench2026} & Behavioral safety across web, mobile, and embodied settings, with partial controllability coverage. \\
SafeArena~\cite{safearena2025} & Partial controllability evidence through refusal on harmful tasks. \\

\midrule
\multicolumn{2}{@{}p{\textwidth}@{}}{\textbf{Accountability}\par
\textit{Metrics}: Audit Coverage Ratio (ACR); Trajectory Pass Rate}\\
\addlinespace[2pt]
\openclawmark ATBench-Claw~\cite{atbenchclaw2026} & Accountability-adjacent evidence through taxonomy-based diagnosis of unsafe trajectories; not a direct auditability metric, but relevant to failure attribution and to the paper's explicit ``Compliance, Legal, and Auditability Harm'' category. \\
\openclawmark ClawAudit~\cite{clawaudit2026} & OpenClaw trajectory audit across six risk dimensions; reports a 58.9\% overall pass rate. \\
\openclawmark OpenClaw PRISM~\cite{openclawprism2026} & Lifecycle-hook logging with session-scoped risk accumulation and TTL decay. \\
OS-Sentinel~\cite{ossentinel2025} & Runtime monitoring evidence for attributable block / review decisions in mobile GUI workflows. \\
\end{longtable}
\normalsize
...
\end{align}
where $\mathcal{T}_{\mathrm{ipi}}$ is the set of IPI attack trials. This decomposition reveals that agents may appear robust when measured by coarse ASR yet still exhibit high response rates ($>$93\%), indicating partial susceptibility to adversarial manipulation. \textbf{ClawTrap}~\cite{clawtrap2026} further extends attack-side evaluation to the network layer through a MITM-based red-teaming pipeline that intercepts and tampers with live traffic during agent execution, exposing vulnerabilities invisible to static sandbox evaluation.

\noindent\textbf{Defense-side metrics.}\quad \textbf{SafeClaw-R}~\cite{safeclawr2026} introduces a complementary defense-side paradigm that evaluates whether a safety layer correctly assigns each agent action to one of three verdicts (\textsc{block}, \textsc{review}, or \textsc{pass}) against ground-truth labels, measuring classification accuracy, false-positive rate, detection rate, and block rate. Its quantitative results are discussed in \S\ref{sec:governance}; the key finding for this section is that model-based verdict assignment substantially outperforms rule-based baselines, but the gap between detection and blocking reflects a deliberate conservative design in which uncertain cases are deferred to human review.

Despite this progress on both attack-side and defense-side evaluation, ecosystem coverage remains incomplete. Security-sensitive trajectory diagnosis has broadened through \textbf{ATBench-Claw}, which includes supply-chain compromise, session-state contamination, approval bypass, and cross-tool attack chaining in OpenClaw traces~\cite{atbenchclaw2026}, but its methodology is closer to post-hoc audit / diagnosis than to canonical attack-side benchmarking. No current benchmark yet captures ecosystem-scale marketplace dynamics or cross-agent worm propagation of the kind documented in Section~6.
...
\label{eq:leakage}
\end{equation}
\textbf{PASB}~\cite{doubleagent2026} is the first benchmark to expose memory-privacy vulnerabilities through adversarial attack simulation in personalized CUA systems, defining extraction and tampering metrics over both short-term memory (STM) and long-term memory (LTM):
\begin{align}
...
\end{align}
where $\mathrm{m} \in \{\mathrm{STM}, \mathrm{LTM}\}$, $\mathcal{C}_{\mathrm{m}}$ is the set of test cases for memory type $\mathrm{m}$, and $\mathrm{WSR}$ (Write Success Rate) verifies whether an attacker can overwrite a specified memory marker to a target value in the agent's file system. Note that \textbf{PASB}'s metrics are fundamentally attack-success rates: they measure the adversary's ability to extract or tamper with stored data via indirect prompt injection across 131 threat skills, so its privacy coverage is indirect---it exposes privacy vulnerabilities through a security attack paradigm rather than directly measuring PII leakage, credential retention, or cross-session data isolation. \textbf{GUIGuard}~\cite{guiguard2026} provides the most systematic visual-privacy evaluation to date. Its benchmark, GUIGuard-Bench, contains 630 GUI agent trajectories with 13{,}830 screenshots across Android and PC platforms, each annotated with region-level privacy grounding (bounding-box coordinates and OCR-extracted text) and three fine-grained label dimensions: \emph{risk level} (low / medium / high), \emph{privacy category} (six semantic classes), and \emph{task necessity} (whether the private element is required for task completion). GUIGuard evaluates privacy through a three-stage pipeline---recognition, protection, and task execution under protection---and defines two core metrics: an \emph{element-level recall} measuring how many ground-truth privacy elements are detected, and a strict \emph{end-to-end accuracy} requiring correct detection together with correct risk-level, category, and task-necessity labels for every element. Under this end-to-end criterion, even the strongest proprietary models achieve only 13.3\% on Android and 1.4\% on PC, revealing that privacy recognition---not protection---is the critical bottleneck for current GUI agents. Anonymization-Enhanced Privacy~\cite{anonprivacy2026} and \textbf{MLA-Trust}~\cite{mlatrust2025} contribute supplementary evidence on visual data anonymization and trust-aware memory access, respectively. Beyond these efforts, privacy evaluation for CUA systems remains underdeveloped: no benchmark jointly evaluates memory extraction resistance, runtime PII filtering, and cross-session data isolation under a unified protocol.
...
\label{eq:passkeps}
\end{equation}
where $\mathbf{1}[\cdot]$ is the indicator function. \textbf{ScreenSpot-Pro}~\cite{screenspotpro2025} and \textbf{WorldGUI}~\cite{worldgui2025} evaluate grounding accuracy across diverse interface states. \textbf{MemGUI-Bench}~\cite{memguibench2026} stresses memory-dependent task completion and reveals a 5.2--11.8\% coverage gap relative to standard benchmarks. The grounding-degradation evidence from Section~3.3---where the same agent architecture collapses from near-90\% on static cached pages to 30--61\% on the live web~\cite{onlinemind2web2025}---underscores that reliability evaluation must move beyond static screenshots toward temporally dynamic environments. \textbf{AMA-Bench}~\cite{amabench2025} and \textbf{CUARewardBench}~\cite{cuarewardbench2025} contribute complementary evidence on memory reliability and reward-model calibration. What remains missing is a unified reliability framework that jointly evaluates visual grounding, memory consistency, and action correctness under realistic perturbation conditions.

\blocksubsubsection{Controllability}

Controllability assesses whether the agent respects authority boundaries, follows permission constraints, and supports staged human intervention. \textbf{ST-WebAgentBench}'s \emph{Completion under Policy} (CuP) metric is the clearest operationalization. For a task $\tau$ with policy constraint set $\mathcal{P}$:
\begin{equation}
\mathrm{CuP}(\tau) = \mathrm{TaskSuccess}(\tau) \times \prod_{p \in \mathcal{P}} \mathbf{1}[\mathrm{compliant}(\tau, p)]
\label{eq:cup}
\end{equation}
This formulation credits task completion only when the agent simultaneously adheres to all policy dimensions---including data privacy, content restrictions, and operational compliance~\cite{stwebagentbench2025}. \textbf{BeSafe-Bench} evaluates behavioral safety across four platform domains (web, mobile, embodied VLM, embodied VLA) and nine risk categories, finding that the best agent completes fewer than 40\% of tasks while respecting safety constraints~\cite{besafebench2026}. The \textbf{STEVE} step-verification framework from Section~4 provides task-level controllability measurement, but its coverage is limited to web browsing rather than the full CUA action space~\cite{steve2025}. \textbf{ATBench-Claw} contains approval-sensitive and scope-sensitive trajectory labels in OpenClaw workflows~\cite{atbenchclaw2026}, but these are better read as post-hoc diagnostic slices than as direct controllability metrics. Even so, no current benchmark systematically measures the full controllability design stack described in Section~4---authority scoping, step verification, runtime enforcement, and structural separation---as an integrated evaluation target.

\blocksubsubsection{Accountability}

Accountability evaluates whether agent decisions can be reconstructed, audited, and attributed after execution. We formalize this goal with the \emph{Audit Coverage Ratio} (ACR), a trajectory-level metric over the agent's action trajectory $\mathcal{T} = (a_1, a_2, \ldots, a_n)$:
\begin{equation}
\mathrm{ACR}(\mathcal{T}) = \frac{|\{a_i \in \mathcal{T} : \mathrm{logged}(a_i) \wedge \mathrm{attributable}(a_i)\}|}{|\mathcal{T}|}
\label{eq:acr}
\end{equation}
where $\mathrm{logged}(a_i)$ indicates the action was recorded with sufficient context for reconstruction, and $\mathrm{attributable}(a_i)$ indicates the decision can be traced to a specific input, policy, or reasoning step. This is the least benchmarked of the five goals. \textbf{ClawAudit} provides trajectory-level traces that expose uncertainty under underspecified intent~\cite{clawaudit2026}. OpenClaw PRISM logs session-scoped risk accumulation across lifecycle hooks~\cite{openclawprism2026}. OS-Sentinel contributes runtime monitoring evidence by detecting and blocking unsafe GUI actions before execution~\cite{ossentinel2025}. The \textbf{Clawdbot Safety Audit}~\cite{clawaudit2026} offers the closest precedent for trajectory-based accountability evaluation: it logs complete interaction trajectories---messages, tool-call arguments, and outputs---across six risk dimensions and applies an automated trajectory judge, achieving only a 58.9\% overall pass rate across 34 test cases, with failures concentrated in ambiguous and adversarial scenarios. Notably, its \emph{user-facing deception} dimension---where the agent falsely claims task success or substitutes hallucinated outputs for tool results---exposes a trust violation that spans both reliability and accountability and is not captured by any existing benchmark metric. \textbf{ATBench-Claw} provides adjacent rather than direct accountability evidence: it evaluates safe / unsafe classification and taxonomy diagnosis, not audit-trail completeness, reconstruction fidelity, or attribution quality. Its relevance here is that it treats \emph{Compliance, Legal, and Auditability Harm} as an explicit harm class and operationalizes a form of failure attribution over risk source, failure mode, and harm type~\cite{atbenchclaw2026}. Accountability nevertheless remains the least standardized of the five goals: no benchmark directly measures audit trail completeness, no reproducible test suite scores trajectory reconstruction fidelity, and no formal evaluation isolates how well governance policies translate into observable runtime compliance.
...
\textbf{Capability benchmarks} such as OSWorld~\cite{osworld2024}, Windows Agent Arena~\cite{windowsarena2024}, WebArena~\cite{webarena2023}, WebVoyager~\cite{webvoyager2024}, and AndroidWorld~\cite{androidworld2024} evaluate whether agents can complete open-ended computer tasks under realistic interaction constraints. Online-Mind2Web~\cite{onlinemind2web2025} adds live-web difficulty. These benchmarks primarily measure task success and do not directly assess trustworthiness, but they provide the baseline against which trust-related costs must be evaluated. A system that scores high on SafeArena but fails basic tasks is not meaningfully trustworthy; it is merely inactive. Among capability benchmarks, OSWorld occupies a distinctive role: it is currently the only suite on which essentially every major CUA family---perception--action models, open frameworks, large-scale training pipelines, and commercial systems---has been independently evaluated, and is therefore the de facto cross-system capability anchor used throughout this survey. We elevate it to that role not because it measures trustworthiness (it does not) but because it is the only benchmark that supports a credible cross-family comparison. Table~\ref{tab:osworld-landscape} consolidates reported OSWorld results for the system families covered in Sections~3--7 and shows that capability has progressed from roughly 12\% (GPT-4V at the original release) to above 70\% in under two years~\cite{osworld2024,agents32026,anthropicclaude46}. The capability side continues to widen through Agent~S2~\cite{agents22025}, Agent~S3~\cite{agents32026}, OpenCUA~\cite{opencua2025}, ScaleCUA~\cite{scalecua2025}, ComputerRL~\cite{computerrl2025}, PC Agent-E~\cite{pcaegente2025}, DART-GUI~\cite{dartgui2025}, EvoCUA~\cite{evocua2026}, GTA1~\cite{gta12025}, and Agent Alpha~\cite{agentalpha2026}, which emphasize compositional routing, reinforcement learning, and test-time scaling.
...
\end{tabularx}
\par\smallskip
\noindent\parbox{\columnwidth}{\footnotesize \emph{Note.} OSWorld is the only benchmark on which essentially all of these system families have been independently evaluated, which is why we use it as the cross-system capability anchor. Results are grouped into \emph{published baselines} (numbers reported in the original system papers under the standard OSWorld protocol) and \emph{post-release / variant results} (subsequent updates, best-of-$N$ protocols, subset reports, or vendor-reported numbers). Cross-paper numerical comparisons require caution unless results use the same OSWorld version, a comparable inference budget, and the same observation modality. The table is intended as a coverage map rather than a strict leaderboard. Human baseline on OSWorld is 72.36\%~\cite{osworld2024}.}
\end{table}

\textbf{Safety benchmarks} directly evaluate trustworthiness by measuring whether agents refuse harmful tasks, avoid unsafe execution, and remain policy-compliant under realistic conditions. Representative suites include SafeArena~\cite{safearena2025}, OS-Harm~\cite{osharm2025}, WASP~\cite{wasp2025}, AgentHarm~\cite{agentharm2024}, and ATBench-Claw~\cite{atbenchclaw2026}. ST-WebAgentBench~\cite{stwebagentbench2025}, SecureWebArena~\cite{securewebarena2025}, and OpenAgentSafety~\cite{openagentsafety2025} extend this evidence base to more policy-conditioned and deployment-oriented settings. Together these benchmarks cover prompt injection, social engineering, harmful action execution, policy violation, and, in ATBench-Claw's case, OpenClaw-sensitive trajectory risks spanning tools, skills, sessions, and external actions. BeSafe-Bench~\cite{besafebench2026} extends this to behavioral safety in situated environments spanning web, mobile, and embodied platforms. Personalized-agent evaluations such as From Assistant to Double Agent~\cite{doubleagent2026} sharpen the realism problem further by showing that benchmark assumptions change once the agent is local, user-adapted, and long-horizon. For accountability, however, ATBench-Claw should be read only as adjacent evidence: its core task is safe / unsafe classification plus taxonomy diagnosis, not auditability measurement. Within this evidence type, SafeArena plays a counterpart role to OSWorld: it is the safety benchmark that most directly measures harmful execution, refusal behavior, and the resulting normalized safety score, on a panel of widely deployed base models. Table~\ref{tab:safearena-landscape} reports SafeArena's per-model results. We elevate it to anchor status not because its coverage is broad---it is in fact narrow, with only five evaluated models---but because its three-axis measurement makes the safety/capability trade-off legible in a way that purely binary attack-success rates do not. This evidence type provides the strongest current coverage of the security goal but remains sparse for privacy, controllability, and accountability, and in particular almost none of the capability-frontier systems in Table~\ref{tab:osworld-landscape} appear in Table~\ref{tab:safearena-landscape}.
...
\textbf{Memory and personalization benchmarks} evaluate how cross-session state affects agent behavior. Truly CUA-native memory evaluation remains scarce and is currently limited to two layers: at the policy layer, MemGUI-Bench~\cite{memguibench2026} stresses memory-dependent GUI tasks, with 89.8\% of its 128 mobile-app tasks requiring cross-temporal or cross-spatial memory retention; at the judge layer, CUARewardBench~\cite{cuarewardbench2025} evaluates whether reward models can correctly score memory-conditioned trajectories. These benchmarks do not yet systematically evaluate privacy risks from retained data or poisoning vulnerabilities in memory stores. ATBench-Claw partially narrows that gap by treating persistent memory / session-state contamination as a standardized trajectory-safety risk source in OpenClaw workflows~\cite{atbenchclaw2026}, but it is not yet a dedicated memory-trust benchmark with explicit longitudinal retention, leakage, or poisoning-persistence metrics.

\textbf{Ecosystem evaluation harnesses} have emerged from the OpenClaw community as a response to the need for deployment-level evidence. PinchBench~\cite{pinchbench2026} organizes public comparison on real OpenClaw tasks; Claw-Eval~\cite{claweval2026} tightens the evidence bar with human-verified tasks and repeated-trial pass criteria; ClawBench~\cite{clawbench2026} favors fixed fixtures and deterministic scoring. ResearchClawBench~\cite{researchclawbench2026} extends this infrastructure into specialized research domains. The picture is now more nuanced than a pure capability-only story: the harnesses themselves remain centered on capability and pass-rate measurement, but ATBench-Claw adds benchmark-level trust evidence from within the OpenClaw ecosystem by testing trajectory safety over tools, skills, sessions, and external actions, including supply-chain, session-state, approval, and cross-tool failure modes~\cite{atbenchclaw2026}. What remains missing is ecosystem-wide evaluation of marketplace provenance, cross-agent propagation, and longitudinal governance under shared skill ecosystems.

\textbf{Deployment audits and engineering evidence} constitute a fifth evidence type that does not fit the benchmark paradigm but provides indispensable trustworthiness knowledge. Trajectory-level audits~\cite{clawaudit2026}, lifecycle-oriented security analyses~\cite{tamingopenclaw2026}, MCP threat models~\cite{hou2025mcp}, and runtime enforcement logs~\cite{openclawprism2026} all produce evidence that cannot be captured by fixed task suites. Internal Safety Collapse~\cite{internalsafetycollapse2026} shows that frontier models can generate harmful intermediate artifacts during otherwise legitimate workflows when task structure itself requires them, suggesting that benchmark suites centered on explicit malicious intent may miss an important class of deployment risk. This evidence type is essential for the accountability and controllability goals, where standardized benchmarks do not yet exist. ATBench-Claw~\cite{atbenchclaw2026} represents an early step toward closing that gap: by benchmarking whether guardrail models can correctly classify and diagnose OpenClaw trajectories, it effectively upgrades ad hoc trajectory audits into a standardized judge benchmark---shifting deployment evidence from non-reproducible case studies toward reproducible, metric-driven evaluation of the safety monitoring layer itself.
...
The coverage analysis in Section~8.3 reveals structural gaps that cannot be closed by incremental extension of existing benchmarks. Accountability remains weakly benchmarked. Privacy evaluation is critically sparse. Ecosystem-sensitive security evaluation has started to appear, but ecosystem-wide and longitudinal coverage remain absent. These gaps reflect not just missing benchmarks but missing evaluation paradigms.

\smallskip
\noindent\textbf{Longitudinal evaluation.}
Most current benchmarks evaluate bounded-session interactions rather than true longitudinal deployment. Yet the trust risks documented in this survey, including memory staleness (Eq.~\ref{eq:stale}), progressive preference drift, and storage-to-steering attacks~\cite{storagetosteering2026}, are inherently longitudinal. A longitudinal evaluation paradigm would track trust properties across sessions, measuring how memory accumulation, preference drift, and policy evolution affect security, privacy, and reliability over time. PASB and ATBench-Claw provide useful starting points because both already encode cross-stage or session-sensitive risk structure~\cite{doubleagent2026,atbenchclaw2026}, but neither yet measures multi-session degradation, retention drift, or leakage persistence under a fixed long-running agent.
